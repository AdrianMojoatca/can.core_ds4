...
#else //production
#define DELAY_ENDMSG  PORT_TIMER_MS(3)
#endif
...
void to_open(D2d2_Port *port)
{
    ...
    if (hold_bus_off || (port->rxmsg == 0 && !(port->txpending && port->givechance == 0)))
    {
        //itimer(port, RT_MILLI(1));
        port_timer_rewind(port, PORT_TIMER_MS(2));  // wait before opening
        return;
    }

    port->smsgbase.busstate = BUS_OPEN;
    d2d_switch_port_tx(port, CHAR_OPEN);
...
{
    port->smsgbase.busstate = BUS_MASTER_DELAY;
      d2d_switch_port_tx(port, CHAR_ERROR); // announce master state
...
void to_data(D2d2_Port *port)
{
    ...
    if (port->givechance)
    {
        ...
        port_timer_rewind(port, PORT_TIMER_MS(1+(random_byte() & 3))); // 1 to 4 ms variable delay
        return;
    }
...
void d2d2_msg_timeout_handler(D2d2_Port *port)
{
    ...
    case BUS_OPEN:
        ...
        {// txpending message is undelivable
          ...
          port_timer_rewind(port, 20*PORT_TIMER_SECOND);
          port->givechance = 0;
          if(port->txpending)
        ...
        if (port->givechance) // was I giving chance to some slave?
        {
          C_LEAVE;
          port->givechance = 0; // time's up.  no more chance.

          timeout_to_data(port); // start sending if we have something
          break;
        }
    ...
    case BUS_MAINT:
        ...
        {
            port->smsgbase.busstate = BUS_MASTER;   //retrieve the bus state
            break;
        }
...
void d2d2_uart_rx_irq(D2d2_Port *port, UInt8 c)
{
    ...
    case BUS_SLEEP:
        ...
        port->smsgbase.busstate = BUS_MASTER;
        d2d_switch_port_tx(port, CHAR_ERROR); // announce master state
        port_timer_rewind(port, DELAY_WAKEUP);
              break;
                 //fall through
    default:
...
void d2d2_txmsg(D2d2_Port *port, Smsg txmsg)
{
      ...
      if(ptr[1] == PROTO_INTL)
        ...
        C_LEAVE;
        append_crc(txmsg);
    smsg_enq(&port->txpending, txmsg);      
        
    if (port->smsgbase.busstate == BUS_OPEN)
    {// start sending soon
        port->givechance = 1;
        port_timer_rewind(port, PORT_TIMER_MS(1));
        //timeout_to_data(port);
    }
    else if (port->smsgbase.busstate == BUS_SLEEP)
    {
        port->smsgbase.busstate = BUS_MASTER;
        port->givechance = 1;  // we need to give a chance for slaves even here.
        port_timer_rewind(port, PORT_TIMER_MS(10));  // delay before CHAR_OPEN //TODO: measure wakeup timing of master and slave devices
        d2d_switch_port_tx(port, CHAR_ERROR); // announce master state
    }
}
