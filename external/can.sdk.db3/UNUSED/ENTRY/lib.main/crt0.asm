...
;/*****************************************************************************/
;/* $Id: crt0.asm 33258 2015-08-19 12:46:35Z martin.bouchard $
;/*****************************************************************************/
...
;                              A-Task Notes
; 
; Initially, the USR/SYSTEM stack pointer is set to the stop of the heap.  during
; the call to 'os_execute()', the CPU mode is switch from SYSTEM to SUPERVISOR 
; and the supervisor stack is set to that of the SYSTEM mode just prior to the
; switch.  In effect, the kernel initialization will be done using the top of
; heap memory as the stack.  Once initialization is completed and the first
; task is started, successive SUPERVISOR actions will use the dynamically
; allocated stack and the heap memory will become free for use.
; 
; Task stacks are individually allocated when created and each tasks stack
; pointer is maintained separately.
; 
; Tasks execute in SYSTEM mode.
...
PINSEL5_OFS         EQU 0x00            ; PINSEL5  Address Offset
...
Vectors         LDR     PC, Reset_Addr         
                ...
                NOP                            ; Reserved Vector 
;                LDR     PC, IRQ_Addr
                LDR     PC, [PC, #-0x0120]     ; Vector from VicVectAddr (IRQ)
                LDR     PC, FIQ_Addr

Reset_Addr      DCD     App_Reset_Handler
...
FIQ_Addr        DCD     FIQ_Handler 
           ...
           EXPORT App_Reset_Handler
			import fiq_service_vector
App_Reset_Handler  B       Reset_Handler
Undef_Handler      B       Undef_Handler_
PAbt_Handler       B       PAbt_Handler_
IRQ_Handler        B       IRQ_Handler
FIQ_Handler        B       fiq_service_vector

;;;; The SWI #0 instruction is used by A-Task to switch from SYSTEM to SUPERVISOR mode

SWI_Handler     mov pc,lr                      ; Return in SUPERVISOR mode with IRQ disabled

;;;; The Data-Abort Handler is used by the DBG task to detect bad memory dump addresses

                IMPORT  data_abort_flag
                IMPORT  prefetch_abort_flag
                IMPORT  undefined_instruction_flag
				IMPORT  exception_addr
				import 	os_irq_enter
				import 	os_irq_leave


DAbt_Handler    sub   lr,lr,#04                ; Correct return PC as ARM buggers it by adding 4 (sigh, stupid... but that's how it is...)

                stmfd sp!,{r0,lr}              ; Save regs : R0 is temporary , LR is return address

				mov   r0,lr					   ; Save return address
				ldr	  lr,=exception_addr
                str   r0,[lr]                  

                ldr   lr,=data_abort_flag      ; Fetch address of abort flag

                mov   r0,#0x01                 ; Set it to 1 (TRUE)
                strb  r0,[lr]                  ;
                
                ldmfd sp!,{r0,pc}^             ; Restore R0 and return to callers PC and mode

Undef_Handler_  sub   lr,lr,#04                ; Correct return PC as ARM buggers it by adding 4 (sigh, stupid... but that's how it is...)
                ...
                ldr   lr,=undefined_instruction_flag ; Fetch address of flag
...
PAbt_Handler_   sub   lr,lr,#04                ; Correct return PC as ARM buggers it by adding 4 (sigh, stupid... but that's how it is...)
                ...
                ldr   lr,=prefetch_abort_flag      ; Fetch address of abort flag
...
Reset_Handler   
				...
				bl		PLL_Lock_Wait
				bl		crt0_pll_connect
				b		finish_clock_setup

				
				
; IRQ handling  ----------------------------------------------------------------
; These functions handle ISR for fast and slow intterupts
; IRQ handling  ----------------------------------------------------------------
...
isr_fast_0
			stmfd	r13!,{r0,r1,lr}				; save registers
			ldr   r1,=isr_user_func + 4*0 
			b     fast_isr_handling
			
			stmfd	r13!,{r0,r1,lr}				; save registers
			ldr   r1,=isr_user_func + 4*1
			b     fast_isr_handling

			stmfd	r13!,{r0,r1,lr}				; save registers
			ldr   r1,=isr_user_func + 4*2
			b     fast_isr_handling
		  
			stmfd	r13!,{r0,r1,lr}				; save registers
			ldr   r1,=isr_user_func + 4*3
			b     fast_isr_handling

			stmfd	r13!,{r0,r1,lr}				; save registers
			ldr   r1,=isr_user_func + 4*4
			b     fast_isr_handling
			
			stmfd	r13!,{r0,r1,lr}				; save registers
			ldr   r1,=isr_user_func + 4*5
			b     fast_isr_handling
			
			stmfd	r13!,{r0,r1,lr}				; save registers
			ldr   r1,=isr_user_func + 4*6
			b     fast_isr_handling

			stmfd	r13!,{r0,r1,lr}				; save registers
			ldr   r1,=isr_user_func + 4*7
			b     fast_isr_handling
			
			stmfd	r13!,{r0,r1,lr}				; save registers
			ldr   r1,=isr_user_func + 4*8
			b     fast_isr_handling

			stmfd	r13!,{r0,r1,lr}				; save registers
			ldr   r1,=isr_user_func + 4*9
			b     fast_isr_handling
			
			stmfd	r13!,{r0,r1,lr}				; save registers
			ldr   r1,=isr_user_func + 4*10
			b     fast_isr_handling

			stmfd	r13!,{r0,r1,lr}				; save registers
			ldr   r1,=isr_user_func + 4*11
			b     fast_isr_handling

			stmfd	r13!,{r0,r1,lr}				; save registers
			ldr   r1,=isr_user_func + 4*12
			b     fast_isr_handling

			stmfd	r13!,{r0,r1,lr}				; save registers
			ldr   r1,=isr_user_func + 4*13
			b     fast_isr_handling

			stmfd	r13!,{r0,r1,lr}				; save registers
			ldr   r1,=isr_user_func + 4*14
			b     fast_isr_handling
			
			stmfd	r13!,{r0,r1,lr}				; save registers
			ldr   r1,=isr_user_func + 4*15
			b     fast_isr_handling
			
			stmfd	r13!,{r0,r1,lr}				; save registers
			ldr   r1,=isr_user_func + 4*16
			b     fast_isr_handling

			stmfd	r13!,{r0,r1,lr}				; save registers
			ldr   r1,=isr_user_func + 4*17
			b     fast_isr_handling

			stmfd	r13!,{r0,r1,lr}				; save registers
			ldr   r1,=isr_user_func + 4*18
			b     fast_isr_handling

			stmfd	r13!,{r0,r1,lr}				; save registers
			ldr   r1,=isr_user_func + 4*19
			b     fast_isr_handling
			
			stmfd	r13!,{r0,r1,lr}				; save registers
			ldr   r1,=isr_user_func + 4*20
			b     fast_isr_handling

			stmfd	r13!,{r0,r1,lr}				; save registers
			ldr   r1,=isr_user_func + 4*21
			b     fast_isr_handling

			stmfd	r13!,{r0,r1,lr}				; save registers
			ldr   r1,=isr_user_func + 4*22
			b     fast_isr_handling
			
			stmfd	r13!,{r0,r1,lr}				; save registers
			ldr   r1,=isr_user_func + 4*23
			b     fast_isr_handling
			
			stmfd	r13!,{r0,r1,lr}				; save registers
			ldr   r1,=isr_user_func + 4*24
			b     fast_isr_handling
			
			stmfd	r13!,{r0,r1,lr}				; save registers
			ldr   r1,=isr_user_func + 4*25
			b     fast_isr_handling

			stmfd	r13!,{r0,r1,lr}				; save registers
			ldr   r1,=isr_user_func + 4*26
			b     fast_isr_handling

			stmfd	r13!,{r0,r1,lr}				; save registers
			ldr   r1,=isr_user_func + 4*27
			b     fast_isr_handling

			stmfd	r13!,{r0,r1,lr}				; save registers
			ldr   r1,=isr_user_func + 4*28
			b     fast_isr_handling

			stmfd	r13!,{r0,r1,lr}				; save registers
			ldr   r1,=isr_user_func + 4*29
			b     fast_isr_handling

			stmfd	r13!,{r0,r1,lr}				; save registers
			ldr   r1,=isr_user_func + 4*30
			b     fast_isr_handling

			stmfd	r13!,{r0,r1,lr}				; save registers
			ldr   r1,=isr_user_func + 4*31
			b     fast_isr_handling
...
fast_isr_handling
			...
			bl		vic_reset
			ldmfd	r13!,{r0, r1, lr}		; pop registers and reuturn form the routine
			subs	PC, r14, #4
...
; Common slow interrupt handling
slow_isr_handling
...
crt0_pll_feed
				...
				; Mask FIQ and IRQ bits in CPSR (this disables them)
				MRS		R3,CPSR
				ORR		R1,R3,#0x000000C0
				MSR		CPSR_c,R1

				; Do the PLL FEED sequence safely
				MOV		R1, #0xAA
				MOV		R2, #0x55
                STR     R1, [R0, #PLLFEED_OFS]
                STR     R2, [R0, #PLLFEED_OFS]

				; Restore the original FIQ and IRQ bits in CPSR
				MSR		CPSR_c,R3
				POP		{R0-R3}
				MOV		pc,lr
				
; Clock Setup ------------------------------------------------------------------
				EXPORT	crt0_clock_setup
crt0_clock_setup
                
				;_crt0_clock_setup is using registers R0,R1,R2,R3
				;Push thouse register onto stack(including also LR)
				PUSH {R0-R3,R14}
                
				bl _crt0_clock_setup
...
;  Supervisor Stack Pointer is setup by 'os_execute()'
