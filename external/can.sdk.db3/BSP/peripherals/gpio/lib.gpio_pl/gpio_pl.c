...
//CLEAR CAPTURE STATUS FLAG OF REGISTER FOR PORT 0 CHANNEL 0 - Daniel.G
#ifdef PLATFORM_404
void cap_int_clr(Gpio_Pl_T pl)
{
	T0IR = (1<<4);  // clear CR0 interrupt capture flag
}
#endif
//*****************************************************************************************
...
Boolean gpio_pl_int_stat_f(Gpio_Pl_T pl)
{
     ...
     if(pl == GPIO_PL_IN1)
     {	
			if(T0IR & (1<<4)) // Daniel.G - if Capture interrupt on P1[26] CAP0.0 is set due to a edge rising, falling or both test if it's my pl
				return TRUE;
			else
				return FALSE;
     }
